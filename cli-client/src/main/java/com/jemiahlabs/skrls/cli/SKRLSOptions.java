...
public class SKRLSOptions {
    ...
    public static Options getInstance(){
        if(Objects.isNull(options)){
            ...
            Option list = Option.builder("ls").longOpt("list").desc("use with -p option. List installed plugins").build();
            Option remove = Option.builder("r").longOpt("remove").hasArg().argName("PLUGIN").desc("use with -p option. Remove an installed plugin").build();
            Option analyze = Option.builder("a").longOpt("analyze").desc("analyze source files").build();
            Option language = Option.builder("l").longOpt("language").hasArg().numberOfArgs(1).argName("LANGUAGE PLUGIN").desc("use with option -a. Select the plugin language").build();
            Option input = Option.builder("i").longOpt("input").hasArg().numberOfArgs(1).argName("FILE PATH").desc("use with option -a. Specify the input directory").build();
            Option output = Option.builder("o").longOpt("output").hasArg().numberOfArgs(1).argName("FILE PATH").desc("use with option -a. Specify the output directory").build();
